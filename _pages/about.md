...
* 기획, 설계, 개발, 테스트, 배포 등 개발 프로세스 전반에 직접 참여
* 대규모 시스템 개발 프로젝트에 참여, 현장 통합 테스트 진행

해당 경험들로 **안정적인 서비스**의 가치를 배우며 나무보다 숲을 볼 수 있게 되었습니다.  

개인적으로 무언가를 배우는 것을 좋아하여 평상시에 책을 읽거나 강의를 수강하며 학습하고 있습니다.  
...
**백엔드 개발** : Java(Spring Boot), AWS, Git
...
**백엔드 개발** : Java(Spring Boot), PHP(CI), AWS, Bitbucket, Jenkins, Jira
...
**앱 & 백엔드 개발** : Java(Spring Boot), AWS, TypeScript(Ionic)
...
> 
> **문제 해결 사례**
> * 로그 설정시 쌓여가는 로그 파일에 대한 처리를 미적용
> * 해당 EC2 인스턴스 서버 용량의 남은 공간이 없어 서비스의 장애 발생
> * 기준 일자를 선정하여 리눅스 crontab으로 저장된 로그 파일을 압축, 삭제 처리하여 용량 확보
> 
> **배운 점**
> * 서버의 용량은 서비스의 성능과 관련이 깊기 때문에 용량 관리 또한 철저히 해야 함
> * 로그 파일을 해당 서버에 두기보다는 로그 전용 서버나 별도의 파일 서버의 저장하는 것이 좋음
>   * 스케일 아웃으로 여러 서버가 있는 경우 로그 파일 보관, 관리가 어려울 수 있음
>   * 해당 로그 파일을 분석하는 모듈(서비스)을 사용하는 경우 대비
> * 또한 상황에 따라 DB에 필요한 로그 데이터를 저장하는 것도 좋은 방법
>   * 네트워크 등의 문제로 로그 파일 생성도 그대로 수행하는 것이 좋음

---

### Insight (광고 수익 정산 서비스) 유지보수
ADOP (2019.04 ~ 2019.07) 4개월
* Java(Spring Boot, Logback), MyBatis, Gradle, JavaScript(jQuery), AWS(EC2, RDS-Aurora, ACM), Bitbucket, Jenkins
...
> * 하지만 API를 지원하지 않는 경우 운영팀에서 직접 데이터 입력
> * 이로 인해 잘못된 데이터 입력 또는 입력이 중복되는 상황(인적 장애) 발생 
> * Apache POI(오피스 파일 변환 라이브러리) 활용, 엑셀 파일 업로드 방식으로 변경
> 
> **배운 점**
> * 처음엔 완전 자동화를 위해 아닌 크롤링 구현 시도했지만 스크래핑이 차단됨(Timeout)
> * 트래픽 부하, 보안 등을 이유로 특정 IP가 차단되거나 처음부터 허용하지 않음
> * User-Agent, 프록시 IP 설정 방법이나 tor(익명 브라우저)를 사용한 IP 우회 방법도 마찬가지
> * 이를 통해 자사 서비스도 보안 등을 이유로 스크래핑을 차단할 필요가 있다는 것을 확인
...
> * 슬로우 쿼리 기준 시간을 long_query_time으로 설정이 가능하나 기준 시간 판단이 어려움
> * 또한 쿼리의 실행 계획이 인덱스를 사용하지 않는 경우 log_queries_not_using_indexes로 확인 가능
...
> 
> **배운 점**
> * Gradle을 활용해 각 프로젝트를 모듈로 취급하여 병합할 수 있다는 것을 확인
> * 이를 통해 모듈들을 한 곳으로 취합함으로써 관리 포인트 축소 가능
> * 각 모듈의 기능과 의존 라이브러리를 공유하여 이전보다 깔끔한 구조가 됨
