...
* 해당 API 서버 모듈이 의존하는 JPA 엔티티 모듈의 카프카 의존성 제거 리팩터링  
  엔티티 리스너에서 이벤트 전송 기능을 구현하면서 생긴 하위 모듈들의 카프카 의존성 제거를 위해 JPA 엔티티 모듈의 메시지 전송 로직을 인터페이스화하고 해당 구현 부를 API 서버 모듈로 이동
...
* RDS의 log_queries_not_using_indexes 옵션으로 슬로우 쿼리를 찾아 처리  
  RDS 요청 트래픽이 특별히 증가하지 않음에도 CPU 부하가 증가해 슬로우 쿼리를 예상, slow_query_log 설정으로는 진짜 느리지 않은 쿼리까지 기록에 남아 확인이 어려워 log_queries_not_using_indexes 옵션으로 약 10초 이상이 걸리는 인덱스 미사용 쿼리를 찾아 해당 테이블의 인덱스 추가, 쿼리의 조건절을 수정하여 약 1초 미만으로 개선  
